...
- **기본은 REST, 강한 일관성**: /v1, snake_case DB ↔ camelCase API 매핑.
...
export const updateUser =
  (uc: UpdateUserName): RequestHandler =>
  async (req, res, next) => {
    try {
      const input = schema.parse(req.body);
      const u = await uc.exec(input);
...
export const errorHandler: ErrorRequestHandler = (err, _req, res, _next) => {
  const e =
    err instanceof AppError
      ? err
      : new AppError("INTERNAL", "unexpected", 500, err);
  res
    .status(e.status)

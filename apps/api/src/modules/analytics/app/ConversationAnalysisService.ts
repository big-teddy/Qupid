...
export class ConversationAnalysisService {
  ...
  private analyzeCommunication(messages: ConversationMessage[]): {
    score: number;
    feedback: string;
  } {
    const userMessages = messages.filter((m) => m.sender === "user");
  ...
  private analyzeEmpathy(messages: ConversationMessage[]): {
    score: number;
    feedback: string;
  } {
    const userMessages = messages.filter((m) => m.sender === "user");
    ...
    const empathyCount = userMessages.reduce(
      (count, m) =>
        count + empathyWords.filter((word) => m.content.includes(word)).length,
      0,
    );
    ...
    return {
      ...
      feedback:
        feedbacks.length > 0
          ? feedbacks.join(" ")
          : "기본적인 공감 능력을 보이고 있습니다.",
    };
  }

  private analyzeEngagement(messages: ConversationMessage[]): {
    score: number;
    feedback: string;
  } {
    const userMessages = messages.filter((m) => m.sender === "user");
    ...
    const activeCount = userMessages.reduce(
      (count, m) =>
        count + activeWords.filter((word) => m.content.includes(word)).length,
      0,
    );
  ...
  private analyzeTopicDiversity(messages: ConversationMessage[]): {
    score: number;
    feedback: string;
  } {
    const userMessages = messages.filter((m) => m.sender === "user");
    ...
    return {
      ...
      feedback:
        feedbacks.length > 0
          ? feedbacks.join(" ")
          : "기본적인 주제 다양성을 보이고 있습니다.",
    };
  }

  private analyzeQuestionQuality(messages: ConversationMessage[]): {
    score: number;
    feedback: string;
  } {
    const userMessages = messages.filter((m) => m.sender === "user");
    ...
    const openQuestions = questions.filter(
      (q) =>
        q.content.includes("어떻게") ||
        q.content.includes("왜") ||
        q.content.includes("어떤"),
    );

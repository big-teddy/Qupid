...
export class LevelSystemService {
  ...
  calculateUserLevel(
    ...
    const currentLevelReq = this.LEVEL_REQUIREMENTS.find(
      (req) => req.level === currentLevel,
    );
    const nextLevelReq = this.LEVEL_REQUIREMENTS.find(
    ...
    const progressPercentage = Math.round(
      ((totalExp - currentLevelReq.requiredExp) /
        (nextLevelReq.requiredExp - currentLevelReq.requiredExp)) *
        100,
    );
  ...
  private getNextLevel(currentLevel: number): number | null {
    ...
    return this.LEVEL_REQUIREMENTS.find((req) => req.level === nextLevel)
      ? nextLevel
      : null;
  }
  ...
  private calculateAchievements(
    ...
    if (
      conversationCount >= 1 &&
      !achievements.includes("first_conversation")
    ) {
    ...
    if (
      conversationCount >= 1 &&
      !achievements.includes("tutorial_completed")
    ) {
    ...
    if (
      ...
      !achievements.includes("conversation_master")
    ) {
    ...
    if (
      this.getCurrentLevel(
        this.calculateTotalExp(conversationScores, conversationCount),
      ) >= 5 &&

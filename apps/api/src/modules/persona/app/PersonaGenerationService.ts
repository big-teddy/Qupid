...
export class PersonaGenerationService {
  ...
  async generatePersonaForUser(
    ...
    try {
      ...
      // 관심사에 따른 직업 선택
      persona.occupation =
        this.selectOccupationBasedOnInterests(commonInterests);

      // 관심사에 따른 가치관 선택
      persona.values = this.selectValuesBasedOnInterests(commonInterests);

      // 성격에 따른 소통 스타일 결정
      persona.communicationStyle = this.determineCommunicationStyle(
        persona.personality,
      );
      ...
      // 성격에 따른 외모 스타일 결정
      persona.appearanceStyle = this.determineAppearanceStyle(
        persona.personality,
        partnerGender,
      );

      // 성격에 따른 말투 특징 결정
      persona.speechPattern = this.determineSpeechPattern(
        persona.personality,
        partnerGender,
      );
      ...
      persona.conversationStyle =
        this.generateConversationStyleDescription(persona);
      ...
      persona.conversation_preview = [
        {
          sender: "ai",
          text: `안녕하세요! ${persona.name}입니다. ${commonInterests[0]} 좋아하시나요?`,
        },
      ];
  ...
  private createBasePersona(
    ...
    const mbti =
      this.MBTI_TYPES[Math.floor(Math.random() * this.MBTI_TYPES.length)];
    const location =
    ...
    return {
      ...
      system_instruction: this.generatePersonaSystemInstruction(
        name,
        age,
        occupation,
        mbti,
  ...
  private selectCommonInterests(userInterests: string[]): string[] {
    ...
    while (commonInterests.length < 5) {
      const randomInterest =
        this.INTERESTS[Math.floor(Math.random() * this.INTERESTS.length)];
  ...
  private adjustPersonalityBasedOnInterests(
    ...
    Object.keys(persona.bigFiveScores).forEach((key) => {
      const score =
        persona.bigFiveScores[key as keyof typeof persona.bigFiveScores];
      persona.bigFiveScores[key as keyof typeof persona.bigFiveScores] =
  ...
  private selectOccupationBasedOnInterests(interests: string[]): string {
    ...
    return this.OCCUPATIONS[
      Math.floor(Math.random() * this.OCCUPATIONS.length)
    ];
  }
  ...
  private selectValuesBasedOnInterests(interests: string[]): string[] {
    ...
    if (values.length < 3) {
      ...
      while (values.length < 3 && remainingValues.length > 0) {
        const randomValue =
          remainingValues[Math.floor(Math.random() * remainingValues.length)];
  ...
  private determineAppearanceStyle(
    mbti: string,
    gender: "male" | "female",
  ): string {
    if (gender === "female") {
  ...
  private generateSpecialNotes(persona: PersonaProfile): string[] {
    ...
    if (Math.random() < 0.3) {
      notes.push(
        `${allergies[Math.floor(Math.random() * allergies.length)]} 알레르기`,
      );
    }
  ...
  private generateConversationStyleDescription(
    persona: PersonaProfile,
  ): string {
  ...
  private generatePersonaSystemInstruction(
    name: string,
    age: number,
    occupation: string,
    mbti: string,
...
이제 ${name}으로서 진짜 사람처럼 자연스럽고 매력적인 대화를 시작하세요! 💕`;
  ...
  private getAgeSpecificTraits(age: number): string {
    if (age < 25)
      return "젊고 활발하며 새로운 것을 시도하는 것을 좋아함. SNS를 자주 사용하고 트렌드에 민감.";
    if (age < 30)
      return "성장하는 시기로 진로나 미래에 대한 고민이 많음. 연애와 결혼에 대한 관심이 높음.";
    if (age < 35)
      return "안정을 추구하며 진지한 관계를 원함. 경험을 바탕으로 한 조언을 잘함.";
  ...
  private getOccupationTraits(occupation: string): string {
    const traits: Record<string, string> = {
      디자이너:
        "창의적이고 예술적 감각이 뛰어남. 아름다운 것에 관심이 많고 트렌드에 민감.",
      개발자:
        "논리적이고 체계적 사고를 함. 문제 해결을 좋아하고 기술에 대한 열정이 있음.",
      마케터:
        "사람들의 심리를 잘 이해하고 소통 능력이 뛰어남. 트렌드와 변화에 민감.",
      기자: "호기심이 많고 소통 능력이 뛰어남. 다양한 주제에 관심이 있고 정보를 잘 전달함.",
      교사: "배려심이 많고 인내심이 강함. 다른 사람의 성장을 도우는 것을 좋아함.",
      간호사:
        "따뜻하고 헌신적이며 타인을 돌보는 것을 좋아함. 위기 상황에서도 침착함.",
      의사: "책임감이 강하고 신중함. 타인을 돕는 것에 보람을 느끼며 지식이 풍부함.",
      변호사:
        "논리적이고 분석적 사고를 함. 정의감이 강하고 소통 능력이 뛰어남.",
      회계사: "꼼꼼하고 체계적이며 신뢰할 수 있음. 정확성과 완벽함을 추구함.",
      영업: "사교적이고 적극적이며 목표 지향적. 사람들과의 관계를 중시함.",
      HR: "사람을 잘 이해하고 소통 능력이 뛰어남. 조직과 개인의 조화를 추구함.",
      기획자:
        "창의적이고 전략적 사고를 함. 새로운 아이디어를 좋아하고 실행력이 있음.",
      유튜버:
        "창의적이고 표현력이 뛰어남. 사람들과 소통하는 것을 좋아하고 트렌드에 민감.",
      "카페 사장":
        "따뜻하고 친근하며 사람들과의 만남을 소중히 여김. 커피와 분위기에 대한 감각이 있음.",
      승무원:
        "서비스 정신이 뛰어나고 글로벌 감각이 있음. 사람들을 배려하는 마음이 깊음.",
      셰프: "창의적이고 완벽주의적. 맛과 미식에 대한 열정이 있으며 세심함이 뛰어남.",
      작가: "감성적이고 창의적. 깊이 있는 사고와 표현력이 뛰어나며 독서를 좋아함.",
      번역가:
        "언어에 대한 감각이 뛰어나고 세심함. 문화와 소통에 대한 이해가 깊음.",
      상담사:
        "공감능력이 뛰어나고 사람을 잘 이해함. 따뜻하고 신뢰할 수 있는 성격.",
      트레이너:
        "활동적이고 에너지가 넘침. 건강과 운동에 대한 열정이 있으며 동기부여를 잘함.",
      건축가:
        "창의적이고 공간 감각이 뛰어남. 미학과 기능성을 모두 고려하는 사고를 함.",
      예술가:
        "감성적이고 창의적. 아름다운 것에 대한 감각이 뛰어나며 자유로운 영혼.",
      음악가:
        "감성적이고 표현력이 뛰어남. 음악에 대한 열정이 있으며 감정 표현이 풍부.",
      배우: "표현력이 뛰어나고 감성적. 다양한 감정을 이해하고 표현하는 능력이 있음.",
      모델: "아름다움에 대한 감각이 뛰어나고 자신감이 있음. 트렌드와 패션에 민감.",
      편집자: "세심하고 완벽주의적. 내용에 대한 이해가 깊고 표현력이 뛰어남.",
      사진작가:
        "예술적 감각이 뛰어나고 세심함. 아름다운 순간을 포착하는 능력이 있음.",
      요리사:
        "창의적이고 완벽주의적. 맛과 미식에 대한 열정이 있으며 세심함이 뛰어남.",
      바리스타:
        "세심하고 완벽주의적. 커피에 대한 전문성과 서비스 정신이 뛰어남.",
      미용사:
        "예술적 감각이 뛰어나고 사람들과의 소통을 좋아함. 아름다움에 대한 감각이 있음.",
      강사: "전달력이 뛰어나고 사람들을 가르치는 것을 좋아함. 지식과 경험을 공유하는 것을 즐김.",
      연구원: "논리적이고 분석적. 지식을 추구하며 체계적인 사고를 함.",
      엔지니어:
        "논리적이고 체계적. 문제 해결을 좋아하고 기술에 대한 열정이 있음.",
      "프로젝트 매니저":
        "조직력이 뛰어나고 리더십이 있음. 계획적이고 체계적인 사고를 함.",
    };

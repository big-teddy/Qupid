...
test.describe('Coaching Dashboard & Goal Interaction', () => {
    test.beforeEach(async ({ page }) => {
        // Mock API responses
        await page.route('*/**/api/v1/coaches/dashboard?userId=*', async (route) => {
            await route.fulfill({
                json: {
                    success: true,
                    data: {
...
test.describe('Styling Coach Interaction', () => {
        ...
        await page.route('*/**/api/v1/coaches/styling-advice', async (route) => {
            ...
            await route.fulfill({
                json: {
                    success: true,
                    data: {
                        advice: "당신의 스타일은 모던하고 시크합니다! 블랙 자켓을 추천해요.",
    ...
    test('should navigate to styling coach and get advice', async ({ page }) => {
        ...
        // Verify Result
        await expect(page.getByText('당신의 스타일은 모던하고 시크합니다!')).toBeVisible();
    });
});

...
// Use unknown for maximum flexibility with React Query mutations
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type GeneratePersonaMutation = {
    mutateAsync: (params: {
...
const convertToPersona = (result: unknown): Persona => {
    ...
    return {
        ...
        tags: Array.isArray(data.interests) ? (data.interests as string[]).slice(0, 3) : [],
        match_rate: 85,
        system_instruction: `당신은 ${data.name}입니다. 자연스럽고 친근한 대화를 나누세요.`,
        personality_traits: Array.isArray(data.values) ? (data.values as string[]).slice(0, 3) : [],
        interests: Array.isArray(data.interests)
            ? (data.interests as string[]).slice(0, 3).map((topic: string) => ({
                emoji: "✨",
                topic,
                description: `${topic}에 관심이 있어요`,
            }))
            : [],
        conversation_preview: [{ sender: "ai" as const, text: "안녕하세요! 반가워요 😊" }],
    };
};

export const generateTutorialPersona = async (
    profile: NewUserProfile,
    generatePersonaMutation: GeneratePersonaMutation
): Promise<Persona> => {
    try {
        ...
        const result = await generatePersonaMutation.mutateAsync({
            userGender: profile.user_gender as "male" | "female",
            userInterests: interests,
            isTutorial: true,
        });
...
export const createFallbackPersona = (profile: NewUserProfile): Persona => {
    const partnerGender: "male" | "female" = profile.user_gender === "male" ? "female" : "male";
    const interests = profile.interests.map((i: string) => i.split(" ")[1] || i);

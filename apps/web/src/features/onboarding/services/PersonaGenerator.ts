...
const convertToPersona = (result: unknown): Persona => {
  ...
  return {
    ...
    tags: Array.isArray(data.interests)
      ? (data.interests as string[]).slice(0, 3)
      : [],
    match_rate: 85,
    system_instruction: `당신은 ${data.name}입니다. 자연스럽고 친근한 대화를 나누세요.`,
    personality_traits: Array.isArray(data.values)
      ? (data.values as string[]).slice(0, 3)
      : [],
...
export const createFallbackPersona = (profile: NewUserProfile): Persona => {
  const partnerGender: "male" | "female" =

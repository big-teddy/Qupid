...
const OnboardingHeader: React.FC<{
...
}> = ({ onBack, progress, title, questionNumber }) => (
  <div className="absolute top-0 left-0 right-0 px-4 pt-4 z-10">
    ...
    {title && (
      <div className="mt-4">
        {questionNumber && (
          <p className="text-lg font-bold text-[#F093B0]">{questionNumber}</p>
        )}
...
const CheckableCard: React.FC<{
  icon?: string;
  title: string;
  subtitle?: string;
  checked: boolean;
  onClick: () => void;
}> = ({ icon, title, subtitle, checked, onClick }) => (
  <button
    onClick={onClick}
  ...
  >
    ...
    <div className="flex-1">
      <p className="text-xl font-bold" style={{ color: "#191F28" }}>
        {title}
      </p>
      {subtitle && (
        <p className="text-base mt-1" style={{ color: "#8B95A1" }}>
          {subtitle}
        </p>
      )}
    </div>
...
const IntroScreen: React.FC<{
  ...
  onLogin: () => void;
  progress: number;
}> = ({ onNext, onLogin, progress }) => {
  console.log("!!! RENDERING FROM TSX FILE !!!");
  ...
  return (
    <div className="flex flex-col h-full w-full animate-fade-in p-6">
      ...
      >
        <button
          onClick={() => {
            console.log("회원가입하기 버튼 클릭됨!");
            onLogin(); // 회원가입/로그인 화면으로 이동
          }}
          ...
          <div className="mt-8 space-y-3">
            {options.map((opt) => (
              <CheckableCard
                ...
                data-testid={`survey-option-${opt.title}`} // Add data-testid here
              />
            ))}
          </div>
        </main>
...
export const OnboardingFlow: React.FC<{
  onComplete: (profile: NewUserProfile, tutorialPersona?: any) => void;
  onLogin: () => void;
  skipIntro?: boolean;  // 회원가입 후 접근 시 인트로 건너뛰기
}> = ({ onComplete, onLogin, skipIntro = false }) => {
  const [step, setStep] = useState(skipIntro ? 1 : 0);  // skipIntro면 성별선택부터
    ...
    try {
      ...
      if (authToken) {
        try {
          ...
          userId = payload.sub; // Supabase JWT의 sub 클레임이 user ID
      ...
      const userProfile: Partial<UserProfile> = {
        id: userId,  // 로그인한 사용자는 auth.users ID 사용
        name: "사용자",
  ...
  const renderStep = () => {
    ...
    switch (step) {
      ...
      default:
        return (
          <IntroScreen onNext={nextStep} onLogin={onLogin} progress={1} />
        );
    }
  };

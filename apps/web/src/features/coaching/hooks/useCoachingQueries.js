...
export const useSendCoachingMessage = () => {
    return useMutation({
        mutationFn: async ({ sessionId, message }) => {
            const response = await api.post(`/coaches/sessions/${sessionId}/messages`, { message });
...
export const useAnalyzeCoachingSession = () => {
    ...
    return useMutation({
        mutationFn: async ({ sessionId, messages, }) => {
            const response = await api.post(`/coaches/sessions/${sessionId}/end`, { messages });
            return response.data;
        },
        onSuccess: () => {
...
export const useUpdateGoal = () => {
    ...
    return useMutation({
        mutationFn: async ({ goalId, status, userId }) => {
            const response = await api.patch(`/coaches/goals/${goalId}?userId=${userId}`, { status });
            return response.data;
        },
        onSuccess: (_, variables) => {
            queryClient.invalidateQueries({
...
export const useCreateGoal = () => {
    ...
    return useMutation({
        mutationFn: async ({ title, userId }) => {
            const response = await api.post(`/coaches/goals?userId=${userId}`, { title });
            return response.data;
        },
        onSuccess: (_, variables) => {
            queryClient.invalidateQueries({

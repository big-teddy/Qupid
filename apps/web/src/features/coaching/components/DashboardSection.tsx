...
export const DashboardSection: React.FC = () => {
    ...
    if (isLoading) return <div className="h-48 flex items-center justify-center text-gray-400">Loading...</div>;
    ...
    return (
        <section className="mb-6">
            <h2 className="text-xl font-extrabold text-[#191F28] mb-4 px-1 flex items-center">
                나의 코칭 리포트 <span className="ml-2 text-sm font-normal text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">Beta</span>
            </h2>
            ...
            <div className="bg-white p-6 rounded-2xl border border-[#F2F4F6] shadow-sm relative overflow-hidden">
                ...
                <div className="flex items-center justify-between mb-6 relative z-10">
                    <div>
                        ...
                        <span className="text-sm text-[#4F7ABA] font-medium">
                            {(dashboard?.stats?.total_level || 1) > 5 ? '소통 마스터' : '소통 꿈나무 🌱'}
                        </span>
                    </div>
                    <div className="flex flex-col items-end">
                        <div className="text-xs text-gray-400 mb-1">Total Exp</div>
                        <div className="font-mono font-bold text-gray-600">
                            {(stats.friendliness + stats.curiosity + stats.empathy).toLocaleString()}
                        </div>
                    </div>
                </div>
                ...
                <div className="pt-5 border-t border-[#F2F4F6] relative z-10">
                    <div className="flex items-center justify-between mb-3">
                        <h3 className="font-bold text-[#191F28] text-sm">이번 주 목표 🎯</h3>
                        <button
                        ...
                        {dashboard?.goals?.length > 0 ? (
                            dashboard.goals.map((g: Goal) => (
                                <div
                                    ...
                                    className={`flex items-center p-3 rounded-lg cursor-pointer transition-all border ${g.status === 'completed'
                                        ? 'bg-gray-50 border-transparent'
                                        : 'bg-white border-gray-100 hover:border-[#DB7093] hover:shadow-sm'
                                        }`}
                                >
                                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center mr-3 transition-colors ${g.status === 'completed'
                                        ? 'bg-green-500 border-green-500'
                                        : 'border-gray-300'
                                        }`}>
                                        {g.status === 'completed' && <span className="text-white text-xs">✓</span>}
                                    </div>
                                    <span className={`text-sm flex-1 ${g.status === 'completed' ? 'text-gray-400 line-through' : 'text-gray-700'}`}>
                                        {g.title}
                                    </span>
                                </div>
                            ))
                            ...
                            <div className="text-center py-4 bg-gray-50 rounded-lg border border-dashed border-gray-200">
                                ...
                                <button onClick={() => setIsAdding(true)} className="text-xs text-[#4F7ABA] font-bold mt-1">목표 설정하기</button>
                            </div>
                        )}
                    </div>
                </div>

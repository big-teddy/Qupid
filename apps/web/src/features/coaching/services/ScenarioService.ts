...
class ScenarioService {
     ...
     * 난이도별 시나리오를 필터링합니다.
     */
    async getScenariosByDifficulty(difficulty: Scenario["difficulty"]): Promise<Scenario[]> {
        return Promise.resolve(ROLEPLAY_SCENARIOS.filter(s => s.difficulty === difficulty));
    }
}

...
const DataExportScreen: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const CheckboxRow: React.FC<{ title: string; defaultChecked?: boolean }> = ({
    title,
    defaultChecked = true,
  }) => (
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        >
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <h2 className="font-bold text-lg text-[#191F28]">
                내 데이터 내보내기
              </h2>
              <p className="mt-1 text-sm text-[#4F7ABA] leading-relaxed">
                연애 박사에서 생성된 모든 데이터를 안전하게 다운로드할 수
                있어요. 개인정보보호법에 따라 요청하신 데이터를 제공합니다.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="p-4 bg-white rounded-2xl border border-[#F2F4F6]">
          ...
          <div className="space-y-3">
            <FileFormatCard
              title="📄 PDF 보고서"
              description="보기 좋게 정리된 요약 보고서"
              size="약 5-10MB"
            />
            <FileFormatCard
              title="📊 Excel 데이터"
              description="상세 데이터를 표로 정리"
              size="약 2-5MB"
            />
            <FileFormatCard
              title="📋 JSON 원본 데이터"
              description="개발자용 원본 데이터"
              size="약 1-3MB"
            />
          </div>
        </section>
        ...
        <section className="text-center text-sm text-[#8B95A1]">
          <p>
            예상 파일 크기:{" "}
            <span className="font-bold text-gray-600">약 8MB</span>
          </p>
          <p>
            예상 소요 시간:{" "}
            <span className="font-bold text-gray-600">30초 ~ 1분</span>
          </p>
        </section>
      </main>
      ...
      <footer className="p-4 bg-white border-t border-[#F2F4F6]">
        {isExporting ? (
          <div className="w-full h-14 rounded-xl bg-gray-200 flex items-center justify-center relative">
            <div
              className="absolute top-0 left-0 h-full bg-[#F093B0] rounded-xl transition-all"
          ...
          <div className="flex space-x-2">
            <button className="flex-1 h-14 bg-[#0AC5A8] text-white text-lg font-bold rounded-xl">
              다운로드 완료!
            </button>
            <button className="flex-1 h-14 bg-gray-600 text-white text-lg font-bold rounded-xl">
              공유하기
            </button>
          </div>
        ) : (

...
const Section: React.FC<{
  ...
  <section className="mt-8">
    <h2 className="text-2xl font-bold text-[#191F28] pb-1 border-b border-gray-200">
      {title}
    </h2>
    {description && (
      <p className="mt-2 text-base text-[#8B95A1]">{description}</p>
    )}
    <div className="mt-4">{children}</div>
  </section>
);

const ColorSwatch: React.FC<{ name: string; hex: string; varName: string }> = ({
  name,
  hex,
  varName,
}) => (
  <div className="flex items-center">
    ...
    <div className="ml-4">
      ...
      <code className="text-xs text-[#4F7ABA] bg-gray-100 px-1 py-0.5 rounded">
        {varName}
      </code>
    </div>
  </div>
...
const ComponentShowcase: React.FC<{
  ...
  children: React.ReactNode;
  widthClass?: string;
}> = ({ title, children, widthClass = "w-full" }) => (
  <div className="mt-4">
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#F9FAFB]">
      <header className="flex-shrink-0 flex items-center p-3 bg-white border-b border-[#F2F4F6]">
        ...
        <h1 className="flex-1 text-center text-xl font-bold text-[#191F28]">
          🎨 디자인 가이드
        </h1>
      ...
      <main className="flex-1 overflow-y-auto p-6">
        <p className="text-lg text-gray-600">
          이 문서는 프로젝트의 일관된 디자인 시스템을 정의합니다. 컴포넌트,
          색상, 타이포그래피 등의 가이드라인을 포함합니다.
        </p>

        <Section
          title="1. 색상 (Colors)"
          description="앱 전체에서 사용되는 색상 팔레트입니다. CSS 변수로 관리됩니다."
        >
        ...
        <Section
          ...
          description="앱의 텍스트 계층 구조입니다. Pretendard (주)와 Inter (부) 폰트를 사용합니다."
        >
          <div className="p-4 bg-white rounded-xl border border-gray-200 space-y-4">
            <div>
              <h1 className="text-3xl font-bold text-[#191F28]">
                H1 - 3xl/bold: 안녕하세요
              </h1>
            </div>
            <div>
            ...
            <div>
              <p className="text-base font-medium text-[#191F28]">
        ...
        <Section
          title="3. 아이콘 (Icons)"
          description="SVG 아이콘은 React 컴포넌트로 관리됩니다."
        >
        ...
        <Section
          ...
          description="재사용 가능한 UI 요소들입니다."
        >
          <ComponentShowcase title="Buttons">
            <button className="h-12 px-6 bg-[#F093B0] text-white font-bold rounded-xl text-lg">
              기본 버튼
            </button>
            <button className="h-10 px-4 bg-[#FDF2F8] text-[#DB7093] font-bold rounded-lg text-sm">
              보조 버튼
            </button>
            <button
              className="h-12 px-6 bg-[#F2F4F6] text-[#8B95A1] font-bold rounded-xl text-lg"
              disabled
            >
              비활성 버튼
            </button>
            <button className="h-12 w-12 flex items-center justify-center bg-[#F093B0] text-white rounded-full">
          ...
          <ComponentShowcase title="Inputs">
            <input
              ...
              className="w-full h-11 pl-4 pr-4 bg-[#F2F4F6] rounded-full focus:outline-none focus:ring-2 focus:ring-[#F093B0]"
            />
            <textarea
              placeholder="성격, 나이, 직업, 취미, 가치관 등을 상세히 적어주세요."
              className="w-full h-24 p-4 border rounded-xl focus:outline-none focus:ring-2 resize-none text-base"
              style={
                {

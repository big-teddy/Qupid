...
const ProfileEditScreen: React.FC<ProfileEditScreenProps> = ({
  ...
  return (
    <div className="flex flex-col h-full w-full bg-white">
      ...
      <main className="flex-1 overflow-y-auto p-6">
        ...
        <div className="flex justify-center items-center mt-8">
          <div className="relative">
            <div className="w-[120px] h-[120px] rounded-full bg-[#FDF2F8] border-2 border-[#F093B0] flex items-center justify-center text-6xl font-bold text-[#F093B0]">
              {initial}
            </div>
            <button
        ...
        <div className="mt-10 space-y-6">
          <div>
            ...
            <p className="mt-2 text-sm text-[#8B95A1]">
              다른 사용자에게 표시되는 이름이에요. ( {nickname.length} / 10 )
            </p>
          </div>
          <div>
          ...
          <div>
            <label className="text-base font-bold text-[#191F28]">
              거주 지역
            </label>
            <div className="mt-2 w-full h-14 px-4 bg-[#F9FAFB] border border-[#E5E8EB] rounded-xl flex items-center justify-between">
        ...
        <div className="mt-10">
          ...
          <div className="mt-4 space-y-4">
            ...
            >
              ...
              <p className="text-base font-medium text-[#8B95A1]">
                {experience} <ChevronRightIcon className="inline w-4 h-4" />
              </p>
            </button>
      ...
      {showExperienceModal && (
        <div
          className="absolute inset-0 bg-black/40 flex justify-center items-center animate-fade-in"
          onClick={() => setShowExperienceModal(false)}
        >
          <div
            className="w-[90%] max-w-sm bg-white rounded-2xl p-6 animate-scale-in"
      ...
      {showActionSheet && (
        <div
          className="absolute inset-0 bg-black/40 flex justify-center items-end animate-fade-in"
        ...
        >
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <button className="w-full text-center text-lg py-3 text-blue-500">
              카메라로 촬영
            </button>
            ...
            <div className="h-px bg-gray-200"></div>
            <button className="w-full text-center text-lg py-3 text-red-500">
              기본 이미지로 변경
            </button>
            <button
              onClick={() => setShowActionSheet(false)}
              className="w-full mt-2 text-center text-lg py-3 bg-gray-100 rounded-lg font-bold text-blue-500"
            >
              취소
            </button>
          </div>

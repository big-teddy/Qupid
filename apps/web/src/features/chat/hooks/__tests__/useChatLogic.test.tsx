...
const { mockStartStreaming, mockUseSendMessage, mockCreateSession } =
  vi.hoisted(() => ({
    mockStartStreaming: vi.fn(),
    mockUseSendMessage: {
      mutateAsync: vi.fn().mockResolvedValue("AI Response"),
    },
...
vi.mock("../../../../shared/hooks/useStreamingChat", () => ({
  useStreamingChat: ({
    ...
    onMessageComplete: (msg: any) => void;
  }) => ({
    isStreaming: false,
    streamingMessage: "",
...
vi.mock("../useChatQueries", () => ({
  ...
  useCoachSuggestion: () => ({
    mutateAsync: vi
      .fn()
      .mockResolvedValue({ reason: "test", suggestion: "test" }),
  }),

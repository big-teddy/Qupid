...
export const useChatLogic = ({
  ...
  useEffect(() => {
        ...
        try {
          ...
          if (isCoaching && partner && "specialty" in partner) {
            ...
            sessionId = await createSessionMutation.mutateAsync({
              personaId:
                partner && "id" in partner
                  ? partner.id
                  : (partner as any)?.name || "unknown",
              systemInstruction:
                roleplayScenario?.systemPrompt ||
    ...
    if ("specialty" in partner) {
      ...
      roleplayScenario &&
      (conversationMode as string) === "roleplay"
    ) {
      initialMessages.push({
        sender: "system",
      ...
      const greeting =
        partner.intro || `안녕하세요! 저는 ${partner.name}입니다.`;
      setTimeout(() => {
        setMessages((prev) => {

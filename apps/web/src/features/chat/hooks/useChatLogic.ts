...
} from "../../coaching/hooks/useCoachingQueries";
...
export const useChatLogic = ({
    ...
    const [currentMode, setCurrentMode] =
        useState<ConversationMode>(conversationMode);
    const [input, setInput] = useState("");
    const [isLoading, setIsLoading] = useState(false);
    const [isAnalyzing, setIsAnalyzing] = useState(false);
    const [realtimeFeedback, setRealtimeFeedback] =
    ...
    // Streaming Hook
    const { isStreaming, streamingMessage, startStreaming, stopStreaming } =
        useStreamingChat({
    ...
    const progressTutorialStep = useCallback(
        (userMessage: string) => {
            if (!isTutorialMode || isTutorialComplete) return;

            const currentStep = TUTORIAL_STEPS[tutorialStepIndex];
            if (userMessage.length >= 5) { // Simple logic for now
    ...
    }, [isCoaching, messages, isTutorialMode, isTutorialComplete, analyzeCoachingMutation, analyzeMutation, onComplete]);
    ...
    useEffect(() => {
                ...
                try {
                    ...
                    if (isCoaching && partner && "specialty" in partner) {
                        ...
                        sessionId = await createSessionMutation.mutateAsync({
                            ...
                            systemInstruction: roleplayScenario?.systemPrompt || partner?.system_instruction || "",
                        });
                    }
        ...
        if ("specialty" in partner) {
        ...
        } else if (roleplayScenario && (conversationMode as string) === "roleplay") {
            initialMessages.push({
                sender: "system",
            ...
            setTimeout(() => {
                setMessages((prev) => {
                    if (prev.some((m) => m.sender === "ai")) return prev;
                    return [...prev, { sender: "ai", text: greeting }];
                });
            }, 500);
        }
        setMessages(initialMessages);
    }, [isTutorial, userProfile, roleplayScenario, conversationMode, partner, isCoaching]);

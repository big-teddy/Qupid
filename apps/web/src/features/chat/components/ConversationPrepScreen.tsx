...
import { Persona, AICoach, ConversationMode } from "@qupid/core";
...
const ConversationPrepScreen: React.FC<ConversationPrepScreenProps> = ({
  partner,
  onStart,
  onBack,
}) => {
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-white animate-fade-in">
      ...
      <main className="flex-1 px-6 overflow-y-auto">
        <h1 className="text-[32px] font-bold text-[#191F28]">
          대화 준비 완료!
        </h1>
        <p className="text-base text-[#8B95A1] mt-2">
          {persona.name}님과의 첫 만남이에요
        </p>
        ...
        <div className="mt-4 p-5 bg-[#FDF2F8] rounded-2xl">
          ...
          <ul className="mt-3 list-disc list-inside space-y-1.5">
            {tips.map((tip, i) => (
              <li key={i} className="text-sm text-[#DB7093] font-medium">
                {tip}
              </li>
            ))}
          </ul>
        ...
        <div className="mt-4 p-5 bg-[#EBF2FF] rounded-2xl">
          ...
          <p className="mt-2 text-sm text-[#4F7ABA] leading-relaxed font-medium">
            대화 중에 AI가 실시간으로 분석해서 좋은 대화일 때 격려해드려요.
            개선점이 있으면 힌트를 주고, 대화 후 상세한 피드백을 제공해요.
          </p>
        </div>

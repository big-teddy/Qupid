...
const NavItem = ({ label, icon, isActive, hasNotification, onClick }) => {
  ...
  return _jsxs("button", {
    onClick: onClick,
    className:
      "flex-1 flex flex-col items-center justify-center p-2 transition-opacity duration-200 ease-out hover:opacity-80",
    children: [
      _jsxs("div", {
        className: "relative",
        children: [
          ...
          hasNotification &&
            _jsx("div", {
              className:
                "absolute top-0 right-[-4px] w-2 h-2 bg-red-500 rounded-full border border-white",
            }),
        ],
      }),
      _jsx("span", {

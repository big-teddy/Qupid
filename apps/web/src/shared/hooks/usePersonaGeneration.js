...
export const useGeneratePersona = () => {
    return useMutation({
        mutationFn: async (data) => {
            const response = await api.post("/personas/generate", data);
...
export const useGenerateDailyPersonas = () => {
    return useMutation({
        mutationFn: async (data) => {
            const response = await api.post("/personas/generate-daily", data);
            return response.data;
        },
        onError: (error) => {
...
export const useRecommendedPersonas = (userGender, limit = 10) => {
    return useQuery({
        ...
        queryFn: async () => {
            const response = await api.get(`/personas/recommended?userGender=${userGender}&limit=${limit}`);
            return response.data;
        },
        enabled: !!userGender,

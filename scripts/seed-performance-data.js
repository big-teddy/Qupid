...
async function seedPerformanceData() {
  try {
    ...
    const conversations = [
      {
        ...
        ended_at: new Date(
          now.getTime() - 6 * 24 * 60 * 60 * 1000 + 25 * 60 * 1000,
        ).toISOString(),
      },
      {
        ...
        ended_at: new Date(
          now.getTime() - 4 * 24 * 60 * 60 * 1000 + 32 * 60 * 1000,
        ).toISOString(),
      },
      {
        ...
        ended_at: new Date(
          now.getTime() - 2 * 24 * 60 * 60 * 1000 + 28 * 60 * 1000,
        ).toISOString(),
      },
      {
        ...
        ended_at: new Date(
          now.getTime() - 1 * 24 * 60 * 60 * 1000 + 18 * 60 * 1000,
        ).toISOString(),
      },
    ];
    ...
    const messageTexts = [
      {
        ...
        ai: "네 정말 좋아요! 이런 날엔 산책하기 딱이죠",
      },
      {
        user: "맞아요, 혹시 좋아하는 산책 코스가 있나요?",
        ai: "한강공원을 자주 가요. 특히 저녁 노을이 예쁘거든요",
      },
      {
        user: "저도 한강 좋아해요! 어느 쪽을 자주 가시나요?",
        ai: "여의도 쪽이요! 벚꽃 시즌엔 정말 예뻐요",
      },
    ];
    ...
    conversations.forEach((conv) => {
      messageTexts.forEach((msg, idx) => {
        messages.push({
          ...
          content: msg.user,
          timestamp: new Date(
            new Date(conv.started_at).getTime() + idx * 4 * 60 * 1000,
          ).toISOString(),
        });
        messages.push({
          conversation_id: conv.id,
    ...
    console.log(
      "📱 You can now check the app to see the real performance data",
    );
  } catch (error) {

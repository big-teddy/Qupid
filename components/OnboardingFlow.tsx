...
const FixedBottomButton: React.FC<{ onClick: () => void; disabled?: boolean; children: React.ReactNode }> = ({ onClick, disabled, children }) => (
    <div className="absolute bottom-0 left-0 right-0 p-4 bg-white" style={{boxShadow: '0 -10px 30px -10px rgba(0,0,0,0.05)'}}>
        <button
            ...
            style={{ backgroundColor: disabled ? '#F2F4F6' : 'var(--primary-pink-main)' }}
        >
            {children}
        </button>
    </div>
...
export const OnboardingFlow: React.FC<{ onComplete: (profile: UserProfile) => void }> = ({ onComplete }) => {
  ...
  const renderStep = () => {
    switch (step) {
      case 0: return <IntroScreen onNext={nextStep} onSkip={() => onComplete(initialProfile)} />;
      ...
      default: return <IntroScreen onNext={nextStep} onSkip={() => onComplete(initialProfile)} />;
    }
  };

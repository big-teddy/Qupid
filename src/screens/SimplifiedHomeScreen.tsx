...
const SimplifiedHomeScreen: React.FC<SimplifiedHomeScreenProps> = ({ 
  ...
  const weeklyGoals = WEEKLY_GOALS.map(goal => ({
    ...goal,
    currentValue: goal.id === 'conversations_5' ? 2 : goal.id === 'score_80' ? 78 : 45
  }));
              ...
              {weeklyGoals.map((goal) => (
                <div key={goal.id} className="flex items-center justify-between">
                  <div className="flex-1">
                       ...
                       <span className="text-sm font-medium text-gray-800">{goal.goalTitle}</span>
                       {goal.completed && (
              ...
              <div className="space-y-3">
                {recentAchievements.map((achievement) => (
                  <div key={achievement.id} className="flex items-center p-3 bg-gradient-to-r from-[#FDF2F8] to-[#EBF2FF] rounded-xl">
                     ...
                     <span className="text-lg">{achievement.achievementIcon}</span>
                   </div>
                   <div className="flex-1">
                     <h4 className="font-medium text-gray-800">{achievement.achievementName}</h4>
                     <p className="text-sm text-gray-600">{achievement.achievementDescription}</p>
                   </div>
